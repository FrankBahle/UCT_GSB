 ...
 * Game Testing - GLA
 * ---------------------------------------------------------------
 * Framework-free API used by two thin adapters:
 *
 *   server.mjs                  local Node server, data/*.json files
 *   netlify/functions/api.mjs   Netlify function, Netlify Blobs
 *
 * Both expose the same JSON contract under /api, so the browser
 ...
 *   POST /api/answers           score one question or round
 ...
 *   GET  /api/admin/users/:id   one user with per-question detail
...
export const USER_COOKIE = "gla_sid";
export const ADMIN_COOKIE = "gla_admin_sid";
...
export function createApi({ catalog, kv, config = {} }) {
    const settings = {
        ...
        adminPassword:
            String(config.adminPassword || "GLA@2026"),

        siteName:
            String(config.siteName || "Game Testing - GLA"),
    ...
    async function handleAdminCsv(context) {
        ...
        if (view === "questions") {
            ...
            users.forEach(user => {
                ...
                const first =
                    row.points.games[DAY_ONE_ID];

                const second =
                    row.points.games[DAY_TWO_ID];
